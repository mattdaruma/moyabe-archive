...
import { BehaviorSubject, expand, first } from 'rxjs';
...
export class FeedComponent implements AfterContentInit {
  ...
  fadePage = 'out'
  fadeList = 'out'
  listGridDelay = new BehaviorSubject<string>('list')
  paginatorFade = 'in'
  constructor(public mybs: MoyabeBlogService, route: ActivatedRoute) { 
    ...
    this.mybs.Settings.subscribe(settings => {
      ...
      this.listGridDelay.pipe(first()).subscribe(listGrid => {
        if(listGrid != settings.displayFeed){
          this.paginatorFade = 'out'
          setTimeout(()=>{
            this.paginatorFade = 'in'
            this.listGridDelay.next(settings.displayFeed)
          }, 150)
        }
      })
    })
